...
struct UPnP
{
	UPnP()
	{
		...
		devlist = upnpDiscover(2000, NULL/*multicast interface*/, NULL/*minissdpd socket path*/, 0/*sameport*/, 0/*ipv6*/, 2,&upnperror);
		if (devlist)
		{
			dev = devlist;
			while (dev)
			...
			descXML = (char*)miniwget(dev->descURL, &descXMLsize, 0, 0);
#else
